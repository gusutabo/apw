...
{ 
    ...
    if (db_delete_task(atoi(argv[2])) == 0)
        ok("Task deleted.");
    else
...
{ 
    ...
    if (db_complete_task(atoi(argv[2])) == 0)
        ok("Marked as complete.");
    else
        error("Failed to update task.");
}

void cmd_list(int argc, char *argv[]) { (void)argc; (void)argv; db_list_tasks(); }
void cmd_help(int argc, char *argv[]) { (void)argc; (void)argv; usage(); }
...
int usage() { 
    ...
    for (int i = 0; i < n; i++)
        printf("  %-5s %-10s %-20s, %s\n", commands[i].alias, commands[i].name, 
